    ...
    fun startGame() {
        ...
        do {
            ...
            if (inputWord == secretWord) {
                println("\nCorrect!")
                return
            } else if (inputWord == "exit") continue else if (!validateWord(inputWord) || !words.contains(inputWord)) {
                println(getValidationErrors(inputWord))
                continue
            }

            println(inputWord.mapIndexed { index, c -> when (true) {
                (secretWord[index] == c) -> c.uppercase()
                (secretWord.contains(c)) -> c.lowercase()
                else -> "_"
            } }.joinToString(""))
