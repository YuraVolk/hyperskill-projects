...
class Matrix(private val matrix: Array<DoubleArray>, private val rows: Int = matrix.size, private val cols: Int = matrix[0].size) {
    ...
    fun determinant(): Double {
        return when { (rows != cols) -> Double.POSITIVE_INFINITY; (rows == 1) -> matrix[0][0]; else -> {
            (0 until cols).sumOf { col -> (if (col % 2 == 0) 1 else -1) * matrix[0][col] * (Matrix(Array(rows - 1) { i ->
                    matrix.slice(1 until rows).map { it.slice(0 until col) + it.slice(col + 1 until cols) }[i].toDoubleArray()
                }).determinant()) } } }
    }

    fun invert(): Any {
        return determinant().let { det ->
            if (det == 0.0 || det == Double.POSITIVE_INFINITY) "This matrix doesn't have an inverse." else {
                (Matrix(Array(rows) { i -> DoubleArray(cols) { j -> (if ((i + j) % 2 == 0) 1 else -1) * (Matrix(Array(rows - 1) { m ->
                            (matrix.slice(0 until i) + matrix.slice(i + 1 until rows))
                                .map { it.slice(0 until j) + it.slice(j + 1 until cols) }[m].toDoubleArray()
                        }).determinant()) } }).mainLineTranspose()) * (1 / det) } }
    }
