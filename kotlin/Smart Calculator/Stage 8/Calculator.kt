...
class Calculator {
    ...
    private fun extractValue(string: String) = string.toBigIntegerOrNull()
        ?: variables.getOrElse(string) { throw IllegalStateException(NO_VARIABLE) }
    private fun parseExpression(expression: String): BigInteger {
        try {
    ...
    private fun parseAssignment(assignment: String): BigInteger? {
        ...
        check(validateVariableName(rightPart) || rightPart.toBigIntegerOrNull() != null) { INVALID_VARIABLE_VALUE }
        check(rightPart.toBigIntegerOrNull() != null || variables.containsKey(rightPart)) { NO_VARIABLE }
        variables[leftPart] = rightPart.toBigIntegerOrNull() ?: variables[rightPart]!!
        return null
    }

    fun parseInput(input: String): BigInteger? = if (input.contains("=")) parseAssignment(input) else parseExpression(input)
}
