...
public record Matrix(Double[][] matrix, int rows, int cols) {
    ...
    public String toString() {
        ...
        DecimalFormat format = maxLeadingSpaces == 0 ? new DecimalFormat("#") : new DecimalFormat("0." + "0".repeat(maxLeadingSpaces));
        format.setDecimalSeparatorAlwaysShown(false);
        int maxNumLength = Arrays.stream(matrix)
                .flatMap(Stream::of)
                .mapToInt(d -> format.format(d).length())
                .max()
                .orElseThrow();
        ...
        return Arrays.stream(matrix).map(row -> Arrays.stream(row)
                        ...
                        .map(num -> String.format("%" + maxNumLength + "s", num))
